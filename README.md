...
- **SMS-inbound gateway** (`sms_inbound_service.py`) — the receiving end of that composer, and also usable completely independently of the app: a health worker can text `REFER <age> <danger signs or free text>` to a fixed number from any phone's ordinary messaging app, no NeoMatCare app required at all. The app additionally composes optional `HID:`/`NAME:`/`REF:` tags into that same message when it has the data — `HID:<hospital id>` links the referral to an *existing* patient instead of creating a duplicate (falls back to creating a new one, noted as such, if the id doesn't match); a hand-typed message without any tags still works exactly as before. Danger signs are best-effort keyword-matched (`heavy bleeding`, `convulsing`, etc., or the app's own codes) against the same `DANGER_SIGN_MENU` USSD uses, so both channels route through the identical engine. Idempotent on Africa's Talking's message id — a provider retry can't create a duplicate referral. The same gateway also accepts `STATUS <hospital id>`, `NOTE <hospital id> <text>`, `HOUSEHOLD <a member's hospital id>`, `VISIT <hospital id> OK`/`VISIT <hospital id> CONCERN <text>`, and `NUTRITION <hospital id>` (mirroring USSD's staff menu items above), replying directly to the sender by SMS since there's no receiving facility to notify for any of them. `NUTRITION` also works from a patient's own number with no Hospital ID needed (`NUTRITION` for her own guidance, `NUTRITION CHILD [name]` for a child's — name only required if she has more than one).
- **USSD referral, routine follow-up, and household/nutrition access** — a `*XXX#` menu (`ussd_service.py` + `apps/referrals/ussd_views.py`, Africa's Talking USSD callback). What a caller sees depends on who they are: a registered Health Worker/Facility Admin phone gets the staff menu below; a phone matching a `Patient.patient_phone_number` gets a separate, single-purpose nutrition menu instead (see "Patient self-service" below) — no app or account needed, just a phone number a health worker recorded once. Staff menu: **1. Emergency Referral** — the flow described above (existing/new patient → danger signs → confirm-before-create); **2. Check Patient Status** — a one-screen, read-only summary for a known Hospital ID; **3. Log Follow-Up Note** — appends a timestamped, attributed note; **4. Household Follow-Up** — reach a household by any member's Hospital ID or by head-of-household name search, see up to 5 members, then per member: status, note, nutrition tips, or log a blunt visit ("no concerns" / "a concern, one line") — real vitals stay app-only (see `apps/cases/household_service.py`). Items 2–4 are read/append-only by design — none can touch a risk level, a referral, or anything clinically load-bearing. **Patient self-service:** a woman calling from her own registered number gets a two-item menu — her own nutrition guidance, and (if her household has one) her child's — reusing the exact same content the app's Nutrition tab shows (`apps/cases/followup_service.py`'s `format_nutrition_tips()`, shared with the staff lookup above so the two entry points can't drift). Session state for the whole menu is held server-side in cache, keyed by Africa's Talking's `sessionId` (`ussd_session.py`) rather than re-parsed from the raw `text` history on every request — this is what makes free-text name/Hospital-ID/note entry safe mid-session. **Requires a shared cache (Redis) in production** — see [Deployment Guide](./DEPLOYMENT.md#redis-required-for-ussd-sessions).

Both the SMS-inbound and USSD paths finish emergency referrals the same way: DRAFT → PENDING transition, which triggers the existing `notify_referral_pending()` SMS signal to the receiving facility — a health worker on either channel gets the same automatic facility notification an in-app referral would trigger. `STATUS`/`NOTE`/`HOUSEHOLD`/`VISIT`/`NUTRITION` don't go through this transition at all — they're read/append-only and reply directly to the sender (or, on USSD, display directly) instead.
...
- [x] ~~**USSD/SMS for routine CHPS follow-up, not just emergencies**~~ — done, including full household follow-up and nutrition guidance:
  - **Check Patient Status** / **Log Follow-Up Note** — read-only summary and a timestamped, attributed note, both keyed by Hospital ID.
  - **Household Follow-Up** — reach a household by any member's Hospital ID or by searching the head-of-household's name (capped at 5 matches; more asks for a narrower name rather than paginating), see up to 5 members, then per member: check status, log a note, nutrition tips, or log a visit. Visit logging is deliberately blunt — "no concerns" or "a concern, described in one line" — not a replacement for the app's real vitals/ANC/growth forms. See `apps/cases/household_service.py`.
  - **Nutrition guidance** — the same content the app's Nutrition tab shows (pregnancy, adult wellness, or child, as appropriate), reachable two ways: a health worker looking up any patient by Hospital ID (via Household Follow-Up, or `NUTRITION <hospital id>` by SMS), or — the higher-value case — **the patient herself**, texting/dialling from her own phone with no app or account needed at all, identified by `Patient.patient_phone_number`. See `apps/cases/followup_service.py`'s `format_nutrition_tips()`, reused by both entry points. Deliberately skips the AI-enhanced local-food phrasing the app's HTTP endpoints use, since that call's latency doesn't suit a live USSD session's ~10s response budget — same underlying curated data, without the personalisation layer.
  - **Deferred, not built:** a third "recent households" access path (in addition to Hospital ID and name search) — see the discussion in this same PR/session for why: it needs a new model and ongoing writes to support a guessed-at usage pattern neither of the other two paths required, versus Hospital ID and name search which just query data that already exists. Cheap to add later if usage shows people are stuck without it; expensive to have carried speculatively from day one.
