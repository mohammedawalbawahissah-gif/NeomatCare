...
- [x] ~~**USSD/SMS for routine CHPS follow-up, not just emergencies**~~ — done, narrow scope as planned: both channels now offer a **Check Patient Status** read-only summary (risk level, ANC visits, EDD, active referral, household food security) and **Log Follow-Up Note** (a timestamped, attributed note appended to the patient's record, visible in the app immediately), both keyed by Hospital ID — no household browsing, no full CRUD over a keypad. See `apps/cases/followup_service.py`, and the USSD/SMS command reference in [Offline-First Design](#-offline-first-design). Full household management (which household, which member, which visit type) remains out of scope for USSD/SMS — that's a browse-and-select problem numbered menus don't suit, and would need its own menu-tree design if picked up later.
