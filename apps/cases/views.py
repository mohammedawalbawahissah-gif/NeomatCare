...
class EmergencyCaseListCreateView(APIView):
    ...
    def get(self, request):
        ...
        serializer = EmergencyCaseListSerializer(cases.order_by('-created_at'), many=True)
...
class EmergencyCaseDetailView(APIView):
    ...
    def patch(self, request, id):
        ...
        user = request.user
        # Superadmin and facility_admin (for their facility) can edit any case
        # Health workers can only edit their own cases
        if user.role == 'health_worker' and case.created_by_id != user.id:
            return Response(
                {"detail": "You can only edit cases you created."},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def delete(self, request, id):
        ...
        if request.user.role != 'superadmin':
            return Response(
                {"detail": "Only superadmins can delete cases."},
                status=status.HTTP_403_FORBIDDEN,
            )
        case, error = self._get_case(id, request.user)
        if error:
